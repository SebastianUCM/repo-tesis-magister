...
Este capítulo establece los fundamentos teóricos que sustentan las decisiones de modelado adoptadas en esta investigación. Se organiza en cuatro dimensiones: los fundamentos electrofisiológicos de la señal electrocardiográfica, los paradigmas disponibles para su modelado matemático, la justificación del uso de funciones Gaussianas y el rol crítico del volumen de datos en los sistemas de diagnóstico asistido.
...
Resulta relevante para esta investigación que la morfología considerada normal no es única. Corrado et al. \cite{corrado2009twelve} documentan que en atletas altamente entrenados la bradicardia sinusal alcanza prevalencias de hasta el 80\%, el bloqueo auriculoventricular de primer grado el 35\% y la repolarización temprana entre el 50\% y el 80\%, sin que ello constituya patología. La frontera entre variante fisiológica y hallazgo patológico depende del contexto demográfico del sujeto, lo que subraya la necesidad de parametrizar dicho contexto en cualquier generador de señales sintéticas.

\subsection{Paradigmas de modelado de la señal electrocardiográfica}

La literatura ofrece tres familias de aproximaciones al modelado de señales ECG, que se distinguen por el nivel de descripción adoptado.

\subsubsection{Modelos biofísicos}

Descienden directamente del formalismo de Hodgkin-Huxley \cite{hodgkin1952quantitative} y de su adaptación cardíaca \cite{noble1962modification}. Describen los mecanismos iónicos subyacentes y poseen, por tanto, el mayor poder explicativo. Su limitación para el propósito de esta investigación es doble: el costo computacional de integrar sistemas de ecuaciones diferenciales acopladas para cada latido, y el hecho de que su salida natural es el potencial de acción celular y no el trazado de superficie, cuya obtención requiere resolver adicionalmente el problema directo de la electrocardiografía.

\subsubsection{Modelos fenomenológicos dinámicos}

Renuncian a describir el mecanismo celular para centrarse en reproducir la forma de la onda. El referente del área es el modelo de McSharry et al. \cite{mcsharry2003dynamical}, que genera la señal mediante un sistema de tres ecuaciones diferenciales ordinarias acopladas cuya trayectoria recorre un ciclo límite tridimensional. El modelo reproduce con realismo la variabilidad de la frecuencia cardíaca y permite prescribir el espectro de potencia de los intervalos R-R.

Los propios autores reconocen dos limitaciones metodológicas en su aproximación: la validación se apoyó en comparación visual sin reportar métricas cuantitativas de error, y los parámetros de los eventos P-QRS-T fueron derivados de un análisis visual subjetivo.

\subsubsection{Modelos paramétricos por descomposición}

Describen la señal como una combinación lineal de funciones elementales cuyos parámetros se ajustan a datos reales. Clifford et al. \cite{clifford2004model} demostraron que la descomposición mediante suma de funciones Gaussianas es computacionalmente eficiente y permite una parametrización precisa de la asimetría de las ondas. Esta propiedad es especialmente relevante para representar morfologías isquémicas donde la alteración del segmento ST resulta crítica.

\subsection{Justificación del uso de funciones Gaussianas}

La elección de la función Gaussiana como primitiva morfológica se sustenta en cinco argumentos metodológicos frente a las alternativas disponibles:

\textbf{Correspondencia anatómico-funcional.} Cada deflexión del ciclo P-QRS-T es un pulso unimodal, con un máximo definido y decaimiento lateral. La Gaussiana es la función más simple que reproduce esta forma con tres parámetros de interpretación fisiológica directa: la amplitud corresponde al voltaje de la deflexión, la posición indica el instante del evento eléctrico, y el ancho representa su duración. Su ajuste produce magnitudes directamente evaluables desde la práctica clínica.

\textbf{Continuidad con el estándar del área.} El modelo dinámico de referencia \cite{mcsharry2003dynamical} emplea términos Gaussianos como forzamiento angular para generar la morfología P-QRS-T. Adoptar esta base conserva la estructura morfológica validada en la literatura, eliminando simultáneamente el costo de integración del sistema de ecuaciones diferenciales.

\textbf{Interpretabilidad frente a bases ortogonales.} Las bases de propósito general (wavelets, polinomios por tramos) minimizan el error de aproximación, pero sus coeficientes carecen de significado clínico. Dado que el objetivo es generar señales con control explícito, la interpretabilidad fisiológica de cada parámetro constituye un requisito técnico ineludible.

\textbf{Eficiencia frente a redes neuronales informadas por la física (PINNs).} Aunque las PINNs \cite{sophiya2025pinns} incorporan restricciones físicas al aprendizaje, requieren disponer de la ecuación diferencial gobernante. Dado que el ECG de superficie no admite una formulación compacta equivalente, y sumado a las dificultades de convergencia en alta dimensión, su aplicabilidad es subóptima para este problema.

\textbf{Extensión asimétrica.} Las deflexiones reales presentan asimetrías características (ej. fase ascendente lenta en la onda T). La Gaussiana asimétrica captura esta propiedad añadiendo un único parámetro adicional por onda (anchos diferenciados respecto al centro), sin comprometer la interpretabilidad del conjunto.
